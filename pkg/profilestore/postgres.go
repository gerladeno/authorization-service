...
func (pg *PG) GetUser(ctx context.Context, phone string) (*models.User, error) {
	query := fmt.Sprintf(`SELECT id, username, phone, created, updated
FROM user_model
WHERE phone = '%s';`, phone)
...
func (pg *PG) UpsertUser(ctx context.Context, user *models.User) error {
	query := `
INSERT INTO user_model (id, username, phone, created, updated)
VALUES ($1, $2, $3, $4, $5)
...
;`
	...
	for i := 0; i < common.GlobalRequestRetries; i++ {
		...
		result, err = pg.db.Exec(
			ctx, query, user.ID, user.Username, user.Phone,
